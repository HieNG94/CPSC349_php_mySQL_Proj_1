...
$petName = $petID = $petType = $petBirth = $petGender = $breed = $chip = $note = "";
$err = "";
...
<div class="container">
    <div class="row" style="padding-top: 20px;">
        <div class="col-8">
            <h2 style="color: grey">&emsp;<u>Pet Information:</u></h2>
        </div>
        <div class="col-4" style="text-align:right;">
            <button type="button" class="btn btn-info" onclick="window.location.href='add.php'">Add</button>
        </div>
    </div>
    <hr class="seperator" style="border-top: 5px solid gray; border-radius: 1px;">
    <br>
    <div id="contentPanel">
...
<script type="text/javascript">
    var addCard = function(name, id, type, birthday, gender, breed, chip, note) {
        var myPanel = $(`
            <div class="card" id="` + id + `">
                <div class="card-header" style="background-color: rgb(229,174,106);">
                    <div class="row">
                        ...
                        <div class="col-4" style="text-align: right;">
                            <button type="button" class="close" data-target="#` + id + `" data-dismiss="modal">x</button>
                        </div>
                    </div>
                </div>
                <div class="card-body" style="padding:10px; background-color: rgb(247,220,151);">
                    <form>
                        <div class="form-group">
                            <div class="row">
                                <div class="col">
                                    <span><b><u>Pet ID:</u></b> #` + id + `</span>
                                    <br>
                                    <span><b><u>Type:</u></b> ` + type + `</span>
                                </div>
                                <div class="col">
                                    <span><b><u>Birthday:</u></b> ` + birthday + `</span>
                                    <br>
                                    <span><b><u>Gender:</u></b> ` + gender + `</span>
                                </div>
                                <div class="col">
                                    <span><b><u>Breed:</u></b> ` + breed + `</span>
                                    <br>
                                    <span><b><u>Microchip:</u></b> #` + id + `</span>
                                </div>
                            </div>
                        </div>
                        <div class="form-group>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['confirm'])) {
        $rID = $_POST['rID'];
        $sql = "SELECT * FROM pet WHERE petID = '$rID';";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) == 0)
            $err = "Pet's ID not found.";
        else {
            $sql = "DELETE FROM pet WHERE petID = '$rID';";
...
$sql = "SELECT * FROM pet WHERE personID = '$personID';";
$result = mysqli_query($conn, $sql);
while ($row = mysqli_fetch_assoc($result)) {
    $petName = strtoupper($row['petName']);
    $petID = $row['petID'];
    $petType = $row['type'];
    $petBirth = $row['petbirthday'];
    $petGender = $row['petgender'];
    $breed = $row['breed'];
    $chip = $row['microchip'];
    $note = $row['note'];
    echo "<script> addCard('$petName', '$petID', '$petType', '$petBirth',
     '$petGender', '$breed', '$chip', '$note'); </script>";
}
            ...
            <center>
                ...
                <div class="modal-body" style="padding: 20px;">
                    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                        <div class="form-group">
                            <input type="text" class="form-control" id="rID" name="rID" style="text-align: center;" placeholder="Re-enter the pet's ID to remove from record">
